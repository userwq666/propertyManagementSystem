      ...
      <template #header>
        <div class="card-header">
          ...
          <el-button-group>
            ...
            <el-button type="success" @click="handleExport" v-permission="['property:owner:export']">
              <Download /> 导出
            </el-button>
          </el-button-group>
        </div>
...
const handleExport = async () => {
  try {
    loading.value = true
    const res = await exportOwner(queryParams)
    const blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `业主数据_${new Date().getTime()}.xlsx`
